...
- `Archive/` keeps its 60 most recent *incremental* exports and never removes a full one (`initial_compact`, `full_diagnostic`), since only those are complete snapshots. Trimming happens after a successful upload. `Reports/` is never trimmed. Dated reports and CSVs are replaced in place, so a second sync on the same day does not leave a duplicate.
