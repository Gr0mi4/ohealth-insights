...
- Drive holds one cumulative `ohealth-metrics.csv` covering every recorded day, one `ohealth-report.md`, and `ohealth-changes.csv` listing values that were later replaced. There are no dated copies: each used to repeat the previous ninety days.
- `Archive/` keeps three generations of the metrics file (about a day, a week and a month old, refreshed by age so a bad write cannot reach all three), its 60 most recent *incremental* exports, and every full export (`initial_compact`, `full_diagnostic`) regardless of age.
